                            ...
                            if has_price:
                                ...
                                if has_lower_limit:
                                    if has_upper_limit:
                                        ...
                                        where += lower_limit + "<=(substring(price, 2,10) + 0)"
                                else:
                                    if has_upper_limit:
                                        where += "(substring(price, 2,10) + 0)<=" + upper_limit
...
#query = "select review number, count(*) from house_review group by review number order by review number;"
query = "select host_location, count(*) from house_info where room_type = 'private room' group by host_location order by host_location desc;"
#query = "select date, count(*) from house_calendar group by date order by date;"
#query = "select available, count(*) from house_calendar where date = '2023-02-25' group by available order by available desc;"
#query = "select available, count(*) from house_calendar where date = '2023-02-25' group by available order by available;"
#query =  "select id, date, maximum_nights from house_info where date = '2023-05-16';"
#query =  "select id, price from house_info where date = '2023-05-16';"
#query = "select id, host_location, date, maximum_nights, price from test_file where host_location = ('Los Angeles, CA') and 90 <= price <= 105;"
#query = "select minimum_nights, id, price from house_info where host_location = 'Los Angeles, CA';"
#query = "select available, count(*) from xxx;"
#query = "Select price from house_info where abc = 'xyz';"
print(is_available(query))
